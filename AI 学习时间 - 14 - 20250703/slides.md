...
2025-07-03 (第14课) @矩阵前端研发二组

---

# 前置课程

[AI学习时间资源汇总](https://doc.weixin.qq.com/doc/w3_AM8APgaIAE0uF3zN8DNSxeXG8Y67j?scode=AJEAIQdfAAofpDF6ISAM8APgaIAE0&isEnterEdit=1)

<img src="/prelude.png" class="h-100" />
...
<img src="/board.png" style="margin: auto" />
...
<img src="/liberty.png" class="h-30" style="margin: auto" />
...
<img src="/mct.png" style="margin: auto" />
...
上面最关键的函数是`MCTSimulation`，这个函数的作用是从给定状态开始，模拟棋局的后续发展。

```ts
const MCTSimulation = (state) => {
    const tree = new MCT(state) // 以给定状态为根节点构建 MCT
    ...
    let counter = 0 // 从当前状态开始尝试预先设定的模拟次数
    while (counter < MAX_SIMULATION_COUNT) iter(tree.root)
}
```
...
<img src="/mct-exploitation.png" class="h-90" style="margin: auto" />

::right::

<img src="/mct-exploration.png" style="margin: auto" />
...
从图可以看出 AlphaGo 包含三个深度学习网络，分别为**快速策略网络**，**增强策略网络**，**价值网络**。
...
<img src="/alphago-architecture.png" style="margin: auto" />
...
<img src="/policy-network-sl.png" class="h-80" style="margin: auto" />
...
<img src="/value-network.png" class="h-90" style="margin: auto" />
...
<img src="/alphago-mct-search.png" class="h-60" style="margin: auto" />

1. **选择**: 选择最大化价值[上图的 $Q + u(P)$]的节点进行推演。
2. **扩展**: 需要扩展一个节点的时候，通过增强策略网络给出一些候选项，并记录下它们的先验概率[上图的$P$]。
